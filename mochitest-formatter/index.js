    ...
    .pipe(
      through2.obj(function (line, _, done) {
        const outputCapture = (prefix, regexp, prefixColor, captureColor) => {
          ...
          this.push(
            captureColor +
              splitLine(capture, prefix.length).trim() +
              RESET_COLOR +
              "\n"
          );
          done();
        ...
        if (line.includes("[ERROR firefox_on_glean")) {
        ...
        if (
          /\d+:\d+\.\d+ INFO Console message: \[JavaScript Error:/.test(line)
        ) {
        ...
        if (/\d+:\d+\.\d+ GECKO\(\d+\) console\.log: /.test(line)) {
          ...
          return outputCapture(
            ...
            regex,
            INFO_COLOR,
            line.includes("!!!") ? DEBUG_COLOR : DIM_COLOR
          );
        }

        if (/\d+:\d+\.\d+ GECKO\(\d+\) console\.error: ?/.test(line)) {
          // "0:05.67 GECKO(43757) console.error:"
          // "0:03.89 GECKO(26343) console.error: There was an error"
          const regex = /\d+:\d+\.\d+ GECKO\(\d+\) (.*)/;
          return outputCapture("   ✕ ", regex, ERROR_COLOR, DIM_COLOR);
        }

        if (/\d+:\d+\.\d+ GECKO\(\d+\) ?/.test(line)) {
          //  0:05.67 GECKO(43757)   Actual events:
          //  0:05.67 GECKO(43757) Array
          //  0:05.67 GECKO(43757)     - 0 = {"category":"translations.panel","extra":{...},"name":"open","timestamp":1275077217}
          //  0:05.67 GECKO(43757)     - 1 = {"category":"translations.panel","extra":{...},"name":"open","timestamp":1275077853}
          //  0:05.67 GECKO(43757)     - 2 = {"category":"translations.panel","extra":{...},"name":"open","timestamp":1275077855}
          //  0:05.67 GECKO(43757)     - 3 = {"category":"translations.panel","extra":{...},"name":"open","timestamp":1275078463}
          //  0:05.67 GECKO(43757)     - length = 4
          const regex = /\d+:\d+\.\d+ GECKO\(\d+\) ?(.*)/;
          return outputCapture("", regex, DIM_COLOR, DIM_COLOR);
        }
